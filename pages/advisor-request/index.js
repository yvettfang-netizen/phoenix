...
Page({
  ...
  submit() {
    ...
    repository.insert('advisorRequests', {
      family_id: this.data.family.id, user_id: user.id, topic: this.data.form.topic,
      preferred_time: this.data.form.preferred_time, note: this.data.form.note, status: 'requested', created_at: isoNow()
    })
    repository.addTimeline(this.data.family.id, 'advisor_contact', `已申请顾问沟通：${this.data.form.topic}`)
    analytics.track('advisor_contact_requested', { userId: user.id, familyId: this.data.family.id, properties: { topic: this.data.form.topic } })
