...
This format is a recommendation for Founder approval. It is not yet an implemented schema decision.
...
- `Family` is the tenant and privacy boundary. Access to family-scoped records requires an active family membership or an explicitly scoped assignment.
- `Student` is the stable person receiving assessment, learning, Academy, and entitlement services. ASKWISE numeric IDs are not Phoenix Student IDs.
- `Guardian` is an adult/person record that can be linked to a User and related to one or more Students. Guardian is not inferred from `families.parent_name`.
