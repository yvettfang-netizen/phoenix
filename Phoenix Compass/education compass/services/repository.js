...
const PREFIX = {
  ...
  partners: 'par', permissions: 'perm', orders: 'ord', reportFeedback: 'fbk'
}
...
function latestAssessmentForStudent(studentId) {
  return where('assessments', (assessment) => assessment.student_id === studentId)
    .sort((a, b) => (b.updated_at || b.created_at || '').localeCompare(a.updated_at || a.created_at || ''))[0] || null
}

function orderForReport(reportId) {
  return where('orders', (order) => order.report_id === reportId)
    .sort((a, b) => (b.updated_at || b.created_at || '').localeCompare(a.updated_at || a.created_at || ''))[0] || null
}
...
module.exports = {
  ...
  familyOverview, latestAssessmentForStudent, orderForReport, resetDemoData
}
