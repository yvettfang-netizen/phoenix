...
恢复后的 HTTP 仍使用 DML 账号，核对咨询、报告 ID/资料版本/报告版本、原件摘要、学生/当前顾问下载、旧顾问/其他学生拒绝及报告导出；撤回检查点不能重新开放材料或报告。数据库包、原件、密钥不上传 artifact。清理仅作用于本次 UUID schema、命名容器及其数据卷、临时文件，不执行 Docker 全局 prune。
