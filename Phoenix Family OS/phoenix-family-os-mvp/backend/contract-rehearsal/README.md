...
状态：`SYNTHETIC REHEARSAL / FOUNDER POLICY ALIGNED / NO PRODUCTION AUTHORITY`
...
3. 每次重复提交重新核验模拟成员关系、监护关系、适用授权、适用权益和映射状态；原成功回执不能绕过撤回。
...
## Founder-approved policy profile

- 一次性最小报告 handoff 只校验 `ASSESSMENT_SCORING`；没有 Journey、Growth Blueprint 或 Timeline 副作用时，不推定或强制 `LONGITUDINAL_GROWTH_RECORD`。
- 创建 Journey 或 Timeline 时，必须校验独立的 `LONGITUDINAL_GROWTH_RECORD`。
- `entitlement_id = null` 代表本演练中的非付费／非受限最小交付；提供 entitlement 时必须是当前有效且 family/subject/service 完全一致。
- Timeline 可见性只允许 `FAMILY | SUBJECT | ADVISOR_SHARED | INTERNAL_RESTRICTED`；本演练的 Compass 事件使用 `SUBJECT`。
- 客户侧顾问词只允许“顾问跟进／预约顾问解读／顾问服务”；内部 request/case/note 状态不作为客户承诺。
