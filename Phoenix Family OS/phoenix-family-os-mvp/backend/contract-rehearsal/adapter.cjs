...
const {
  FAMILIES, USERS, GUARDIAN, NOW, DOMAINS, TIMELINE_VISIBILITIES, ADVISOR_CUSTOMER_COPY,
  fixture, coreFixture, consentRef
...
function validateEnvelope(h, now) {
  ...
  for (const key of ['result_summary', 'human_review', 'audit_context']) {
    requireCheck(isDeepStrictEqual(h[key], sample[key]), 'HANDOFF_PAYLOAD_TOO_BROAD')
  }
  const minimumReportOnly = h.journey_seed === null && isDeepStrictEqual(h.timeline_candidates, [])
  if (!minimumReportOnly) {
    requireCheck(isDeepStrictEqual(h.journey_seed, sample.journey_seed), 'HANDOFF_PAYLOAD_TOO_BROAD')
    requireCheck(isDeepStrictEqual(h.timeline_candidates, sample.timeline_candidates), 'HANDOFF_PAYLOAD_TOO_BROAD')
  }
  ...
  return { family, sample, minimumReportOnly }
}
...
function createRehearsal({ mode } = {}) {
  ...
  function hasLongitudinalSideEffects(h) {
    return h.journey_seed !== null || h.timeline_candidates.length > 0
  }
  function checkContext(h, session) {
    ...
    if (hasLongitudinalSideEffects(h)) {
      checkConsent(h.consent_refs.LONGITUDINAL_GROWTH_RECORD, 'LONGITUDINAL_GROWTH_RECORD', h)
    } else {
      requireCheck(h.consent_refs.LONGITUDINAL_GROWTH_RECORD === null, 'CONSENT_REQUIRED')
    }
    if (h.entitlement_id !== null) {
  ...
  function ingest(h, session = 'parent') {
    return transaction('HANDOFF', (draft, recordContext) => {
      ...
      const events = h.timeline_candidates.map(event_type => ({
        ...
        visibility: 'SUBJECT', recorded_at: now, simulation: true
      }))
      requireCheck(events.every(event => TIMELINE_VISIBILITIES.includes(event.visibility)), 'PERMISSION_DENIED')
  ...
  function requestFollowUp(input, session = 'parent') {
    return transaction('FOLLOW_UP', (draft, recordContext) => {
      ...
      draft.timeline.push({ event_id: `event_${r.advisor_request_id}`, event_type: 'ADVISOR_FOLLOW_UP_REQUESTED',
        primary_family_id: h.primary_family_id, subject_id: h.subject_id, recorded_at: now,
        visibility: 'FAMILY', customer_label: ADVISOR_CUSTOMER_COPY[0], simulation: true })
      return r
    })
  }
