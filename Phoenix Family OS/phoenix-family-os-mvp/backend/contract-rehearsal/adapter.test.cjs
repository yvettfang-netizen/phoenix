...
const {
  DOMAINS, FAMILIES, USERS, STUDENTS, NOW, EXPIRES, TIMELINE_VISIBILITIES,
  ADVISOR_CUSTOMER_COPY, fixture
...
test('shared Compass list is frozen and Health remains reserved', () => {
  assert.deepEqual(DOMAINS, ['EDUCATION', 'IDENTITY', 'WEALTH'])
  assert.throws(() => fixture('HEALTH'))
})
test('timeline visibility and customer advisor vocabulary use only Founder-approved values', () => {
  assert.deepEqual(TIMELINE_VISIBILITIES, ['FAMILY', 'SUBJECT', 'ADVISOR_SHARED', 'INTERNAL_RESTRICTED'])
  assert.deepEqual(ADVISOR_CUSTOMER_COPY, ['顾问跟进', '预约顾问解读', '顾问服务'])
})
for (const domain of DOMAINS) {
  test(`${domain}: accepted synthetic handoff, stable replay, scoped read`, () => {
    ...
    assert.ok(r.readTimeline(FAMILIES.A).every(event => event.visibility === 'SUBJECT'))
  ...
  test(`${domain}: explicit follow-up → assigned advisor → protected note`, () => {
    ...
    const followUpEvent = state.timeline.find(event => event.event_type === 'ADVISOR_FOLLOW_UP_REQUESTED')
    assert.equal(followUpEvent.customer_label, '顾问跟进')
    assert.ok(!JSON.stringify(followUpEvent).includes('ACCEPTED_FOR_TRIAGE'))
  })
}
test('minimum one-time report handoff does not infer longitudinal consent or require entitlement', () => {
  const r = make(), h = signedFixture()
  h.timeline_candidates = []
  h.journey_seed = null
  h.consent_refs.LONGITUDINAL_GROWTH_RECORD = null
  h.entitlement_id = null
  h.result_payload_hash = resultHash(h)
  const receipt = r.ingest(h)
  assert.equal(receipt.status, 'ACCEPTED')
  assert.deepEqual(receipt.timeline_event_ids, [])
  assert.equal(receipt.journey_id, null)
  assert.deepEqual(count(r), [1, 0, 0, 0, 0, 0])
})
test('longitudinal side effects still require longitudinal consent', () => {
  const r = make(), h = signedFixture()
  h.consent_refs.LONGITUDINAL_GROWTH_RECORD = null
  rejected(() => r.ingest(h), 'CONSENT_REQUIRED')
})
test('paid or restricted handoff rejects an unknown entitlement', () => {
  const h = signedFixture()
  h.entitlement_id = 'entitlement_unknown'
  rejected(() => make().ingest(h), 'ENTITLEMENT_REQUIRED')
})
const invalid = [
  ...
  ['missing entitlement cannot bypass longitudinal validation', h => { h.entitlement_id = null; h.consent_refs.LONGITUDINAL_GROWTH_RECORD = null }, 'CONSENT_REQUIRED'],
