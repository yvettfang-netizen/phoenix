...
Page({
  data: {
    ...
    primaryStudent: null, latestTimeline: null, stage: '', nextStep: null, progress: 0,
  ...
  onShow() {
    ...
    const latestEvent = family ? (repository.eventsForFamily(family.id)[0] || null) : null
    const latestTimeline = latestEvent ? { ...latestEvent, dateLabel: dateLabel(latestEvent.date) } : null
    ...
    this.setData({
      user, family, students, primaryStudent, latestReport, latestTimeline, progress, nextStep,
  ...
  goTimeline() { wx.switchTab({ url: '/pages/timeline/index' }) }
})
